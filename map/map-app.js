...
function openStyleModal(id) {
  ...
  document.getElementById('modalColor').value = r.settings.color;
  document.getElementById('modalOpacity').value = r.settings.opacity;
  document.getElementById('modalWeight').value = r.settings.weight;
  document.getElementById('modalLabelEnable').checked = r.settings.label.enabled;
  document.getElementById('modalCluster').checked = r.settings.clustering;
  document.getElementById('modalHeat').checked = r.settings.heatmap;
  ...
  // Label appearance
  document.getElementById('modalLabelSize').value = r.settings.label.size;
  document.getElementById('modalLabelColor').value = r.settings.label.color;
  document.getElementById('modalLabelHalo').checked = r.settings.label.halo;
...
function applyStyleModal(closeAfter = false) {
  ...
  // Save updated values
  r.settings.color = document.getElementById('modalColor').value;
  r.settings.opacity = parseFloat(document.getElementById('modalOpacity').value);
  r.settings.weight = parseFloat(document.getElementById('modalWeight').value);
  r.settings.label.enabled = document.getElementById('modalLabelEnable').checked;
  r.settings.label.field = document.getElementById('modalLabelField').value;
  r.settings.label.size = parseInt(document.getElementById('modalLabelSize').value);
  r.settings.label.color = document.getElementById('modalLabelColor').value;
  r.settings.label.halo = document.getElementById('modalLabelHalo').checked;
  r.settings.clustering = document.getElementById('modalCluster').checked;
  r.settings.heatmap = document.getElementById('modalHeat').checked;
...
// --------------------------------------------------
// 1. GEOJSON POPUP – use renderMedia() for everything
// --------------------------------------------------
function bindGeoJSONPopup(feature, layer) {
  ...
  const props = {};
  for (let k in raw) props[k.toLowerCase()] = raw[k];

  const name       = props.name       || 'Unnamed feature';
  const desc       = props.descriptio || '';          // intentional column name
  ...
  html += `<h3 style="margin:0 0 8px; font-size:16px; color:#1a1a1a;">${escapeHtml(name)}</h3>`;

  if (cls)       html += `<div><strong>Class:</strong> ${escapeHtml(cls)}</div>`;
  if (area)      html += `<div><strong>Area:</strong> ${escapeHtml(area)}</div>`;
  if (length)    html += `<div><strong>Length:</strong> ${escapeHtml(length)}</div>`;
  if (perimeter) html += `<div><strong>Perimeter:</strong> ${escapeHtml(perimeter)}</div>`;
  if (desc)      html += `<div style="margin:8px 0; line-height:1.4;">${escapeHtml(desc)}</div>`;

  // ---- MEDIA (16:9 containers) ----
  if (props.image_url) html += renderMedia(props.image_url);
  if (props.video_url) html += renderMedia(props.video_url);
  if (props.audio_url) html += renderMedia(props.audio_url);

  if (contact)   html += `<div style="margin-top:8px;"><strong>Contact:</strong> ${escapeHtml(contact)}</div>`;
  html += `</div>`;

  // Wider popup for the iframe
